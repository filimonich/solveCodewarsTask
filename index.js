...
// функця convert, которая принимает три аргумента,
// входное значение (input), исходный алфавит (source) и целевой алфавит (target).
function convert(input, source, target) {
  // определяем длину исходного алфавита
  let sourceLength = source.length;
  // определяем длину целевого алфавита
  let targetLength = target.length;
  // инициализируем переменную для хранения десятичного
  // представления входного значения
  let decimal = 0;
  // запускаем цикл по каждому символу во входном значении
  for (let i = 0; i < input.length; i++) {
    // преобразуем каждый символ в его десятичное значение и
    // добавляем его к общему десятичному значению
    decimal = decimal * sourceLength + source.indexOf(input[i]);
  }
  // объявляем функцию для преобразования десятичного
  // числа в целевую систему счисления
  function convertToTarget(decimal) {
    // если десятичное число равно нулю, возвращаем пустую строку
    if (decimal === 0) {
      return '';
    } else {
      // иначе рекурсивно вызываем функцию с целой частью от деления
      // десятичного числа на основание целевой системы и
      // добавляем к результату символ из целевого алфавита,
      // соответствующий остатку от деления десятичного числа на основание целевой системы
      return (
        convertToTarget(Math.floor(decimal / targetLength)) +
        target[decimal % targetLength]
      );
    }
  }
  // вызываем функцию преобразования для получения результата
  // в целевой системе счисления
  let result = convertToTarget(decimal);
  // если результат пуст, возвращаем первый символ из
  // целевого алфавита, иначе возвращаем результат
